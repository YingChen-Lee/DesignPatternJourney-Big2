...
RankEnum GetRepresentativeRank(RankEnum start_rank) {
    return static_cast<RankEnum>(std::min(static_cast<int>(start_rank) + 4, kTotalRankCount - 1));
}
}  // namespace
...
TurnMove SmallestFirstStraight::PlayWithClub3(std::shared_ptr<Player> player) {
    /*
    ...
    */
    ...
    for (const auto& start_rank : possible_start_ranks) {
        if (std::find(start_ranks_.begin(), start_ranks_.end(), start_rank) != start_ranks_.end()) {
            ...
            assert(Utility::ContainsClub3(cards_to_play) &&
                    "You should have a club 3 if you want to play a Pair with club 3.");
            return Utility::DrawCardsAndPlay(player, cards_to_play);
        }
    }
    assert(false && "You should have a club 3 if you want to play a Pair with club 3.");
}

TurnMove SmallestFirstStraight::PlaySmallest(std::shared_ptr<Player> player) {
    auto cards_to_play = GetCardsToPlay(start_ranks_[0]);
    return Utility::DrawCardsAndPlay(player, cards_to_play);
}

TurnMove SmallestFirstStraight::PlayBiggerThanTopPlayOrPass(std::shared_ptr<Player> player, const std::shared_ptr<CardPattern> top_play) {
